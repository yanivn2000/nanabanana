...
export function BudgetPanel({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div>
        <label className="mb-1 block text-[14px] font-medium">תקציב יומי למשפחה (€)</label>
        <input type="number" min={0} inputMode="numeric"
          value={target ?? ""} placeholder="לא הוגדר"
          onChange={(e) => onChange({ dailyTarget: e.target.value ? Number(e.target.value) : undefined })}
          className="w-32 rounded-lg bg-[var(--surface-2)] px-3 py-2 text-[15px] outline-none" />
      </div>

      {days === 0 ? (
        <p className="text-[14px] text-[var(--text-3)]">בנו לו״ז כדי לראות הערכת עלויות מפורטת.</p>
      ) : (
        <>
          <div className="rounded-[var(--radius-sm)] bg-[var(--surface-2)] p-3">
            <div className="flex items-baseline justify-between">
              <span className="text-[14px] text-[var(--text-2)]">הערכת עלות לטיול ({days} ימים)</span>
              <span className="text-[18px] font-bold">{eur(est.total)}</span>
            </div>
            <div className="mt-1 flex items-center justify-between text-[13px] text-[var(--text-3)]">
            ...
            <div className="mt-2 flex gap-4 text-[13px] text-[var(--text-3)]">
          ...
          <div className="flex flex-col">
            {est.perDay.map((d, i) => (
              <div key={i} className="flex items-center justify-between border-b border-[var(--border)] py-1.5 text-[13.5px]">
