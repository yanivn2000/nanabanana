...
export default function RecommendPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-[440px] px-5 pb-28 pt-8 lg:max-w-2xl lg:px-8">
      ...
      <div className="mt-4">
        <label className="mb-1.5 block text-[14px] text-[var(--text-2)]">מתי בערך? (לעונה)</label>
      ...
      <button onClick={recommend} disabled={status === "loading"}
        className="mt-5 flex w-full items-center justify-center gap-2 rounded-full bg-[var(--brand)] py-3 text-[16px] font-medium text-white disabled:opacity-50">
        {status === "loading" ? <Loader2 size={17} className="animate-spin" /> : <Sparkles size={17} />}
        {status === "loading" ? "חושב לאן…" : "המליצו לי יעד"}
      </button>
      {status === "error" && <p className="mt-2 text-[13.5px] text-[var(--amber)]">{msg}</p>}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[13px] text-[var(--text-3)]">{r.country_he || r.country}</span>
                  </div>
                  {r.highlights && <p className="mt-1 text-[13.5px] font-medium text-[var(--brand-ink)]">{r.highlights}</p>}
                  <p className="mt-1 text-[14.5px] text-[var(--text-2)]">{r.reason}</p>
                </div>
